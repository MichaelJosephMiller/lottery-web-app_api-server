...
module.exports.upsert_drawing_by_date = drawing => {
  return new Promise((resolve, reject) => {
    MegaMillionsHistory.findOneAndUpdate({
      date: drawing.date
    }, drawing, {
      upsert: true
    }, err => {
...
module.exports.insert_multiple_drawings = async drawingsArray => {
  let docs = await MegaMillionsHistory.insertMany(drawingsArray, {
    ordered: false
  }).then(docs => docs).catch(err => console.log(err));
