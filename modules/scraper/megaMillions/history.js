...
module.exports.get = async (limit = 5000) => {
  let response = await axios.get('https://data.ny.gov/resource/5xaw-6ayf.json', { params: { $limit: limit } })
...
function formatted(drawingsArray) {
  let newDrawingsArray = drawingsArray.map(drawing => {
    let newDrawing
    try { newDrawing = new MegaMillionsDrawing(drawing) }
    catch(err) {
  ...
  newDrawingsArray.filter(drawing => { drawing !== {} })
